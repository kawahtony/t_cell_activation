...
P_bind_c_set = zeros(length(lambdaset), 1)' ; 
tau_bind_c_set = zeros(length(lambdaset), 1)' ; 
...
for i = 1:length(lambdaset)
    ...
    disp([num2str(i), ' / ', num2str(length(lambdaset))])

end


filename = 'lambda.mat' ;
save(filename, 'lambdaset', 'P_bind_c_set', 'tau_bind_c_set') ;
