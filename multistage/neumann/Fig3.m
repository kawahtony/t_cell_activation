...
for j = 1:length(Nset)
    ...
    semilogy(pp(abs(qq-q) < 1e-5), TT(abs(qq-q) < 1e-5), style_set{j}, 'Color', cl_set{j}, ...
...
leg = legend ; 
set(leg, 'Interpreter', 'Latex', 'Box', 'off', 'FontSize', 25, 'Position', [0.0900    0.6236    0.0822    0.2328])
...
for j = 1:length(Nset)
    N = Nset(j) ;
    load(['N=', num2str(N), '.mat']) ;
    plot(pp(abs(qq-q) < 1e-5), TT(abs(qq-q) < 1e-5), style_set{j}, 'Color', cl_set{j}, ...
                 'LineWidth', 3, 'DisplayName', sprintf('$N=%i$', N)) ; hold on ;   
end
...
% exportgraphics(gcf, 'tau_vs_N.pdf')
