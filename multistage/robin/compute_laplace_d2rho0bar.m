function [val, res] = compute_laplace_d2rho0bar(D, kappa, f, k_01, mu_0, N, ...
...
while abs(res) > tol
    ...
    d2Khat = 0 ;
    for j = 1:N
        d2Khat = d2Khat - 2*k_01*Vnmat(1,j)*Vninvmat(j,1)/lambda_n_vec(j)^3 ;
    end
    for j = 1:N-1
        d2Khat = d2Khat - 2*f*k_01*Vcmat(1,j)*Vcinvmat(j,1)/lambda_c_vec(j)^3 ;
    end     

    % d2That
    d2That = 2*(dKhat - 1)^2/(An - Khat)^3 + d2Khat/(An - Khat)^2 ;


    % Spatial component Bn
    alpha = nu/sqrt(D) ;
    Bn = 4*kappa*sin(alpha)/(2*alpha - sin(2*alpha)) ;
